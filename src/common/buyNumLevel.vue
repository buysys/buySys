<template>
	<div class="container">
		...
		<!-- 重新分配-->
		<el-dialog title="买号等级分配信息" :visible.sync="accountModel" width="90%" :close-on-click-modal="false" :before-close="closeBuyNum" :modal-append-to-body="false" :append-to-body="true">
			<el-collapse-transition>
			...
			<el-table :data="buyNumData" border style="width: 100%" :default-sort="{prop: 'Numbers', order: 'descending'}" @selection-change="buyHandleSelectionChange">
				...
				<el-table-column label="操作" align="center">
					<template slot-scope="scope">
						<el-button size="small" type="primary" @click="prohibitHandle">禁用</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<!-- 修改参数值-->
		<el-dialog title="修改参数值" :visible.sync="paramModel" :close-on-click-modal="false" width="20%"  :modal-append-to-body="false" :append-to-body="true">
		...
		<!--禁用-->
		<el-dialog title="系统提示" :visible.sync="prohibitModel"  :close-on-click-modal="false" center width="20%"  :modal-append-to-body="false" :append-to-body="true">
			<div class="del-dialog-cnt textCen">确认要禁用吗？</div>
			<span slot="footer" class="dialog-footer">
        		<el-button type="primary" size="medium">确定</el-button>
        		<el-button @click="prohibitModel=false" size="medium">取消</el-button>
      		</span>
		</el-dialog>
	</div>
</template>
